package com.example.evmobile.data.local.dao
...
interface UserDao {
    ...
    @Query("SELECT * FROM users WHERE isLoggedIn = 1 LIMIT 1")
    fun getLoggedInUserLive(): LiveData<UserEntity?>
...
interface ChargingStationDao {
    ...
    fun getAllStationsLive(): LiveData<List<ChargingStationEntity>>
    ...
    @Query("SELECT * FROM charging_stations WHERE id = :stationId")
    fun getStationByIdLive(stationId: String): LiveData<ChargingStationEntity?>
    ...
    @Query("SELECT * FROM charging_stations WHERE isFavorite = 1")
    fun getFavoriteStationsLive(): LiveData<List<ChargingStationEntity>>
...
interface ConnectorDao {
    ...
    @Query("SELECT * FROM connectors WHERE stationId = :stationId")
    fun getConnectorsByStationIdLive(stationId: String): LiveData<List<ConnectorEntity>>
...
interface BookingDao {
    ...
    @Query("SELECT * FROM bookings WHERE userNic = :userNic ORDER BY createdAt DESC")
    fun getBookingsByUserLive(userNic: String): LiveData<List<BookingEntity>>
    
    @Query("SELECT * FROM bookings WHERE id = :bookingId")
    suspend fun getBookingById(bookingId: String): BookingEntity?
    
    @Query("SELECT * FROM bookings WHERE id = :bookingId")
    fun getBookingByIdLive(bookingId: String): LiveData<BookingEntity?>
    ...
    @Query("SELECT COUNT(*) FROM bookings WHERE userNic = :userNic AND status = 0")
    fun getPendingBookingsCountLive(userNic: String): LiveData<Int>
    
    @Query("SELECT COUNT(*) FROM bookings WHERE userNic = :userNic AND status = 1 AND startTime > datetime('now')")
    fun getApprovedBookingsCountLive(userNic: String): LiveData<Int>
...
interface AppSessionDao {
    ...
    @Query("SELECT * FROM app_sessions WHERE id = 'current_session'")
    fun getCurrentSessionLive(): LiveData<AppSessionEntity?>
